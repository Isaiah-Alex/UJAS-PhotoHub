...
export function BookingsTab({
  ...
  return (
    <div className="space-y-3 md:space-y-6 overflow-scroll pr-2 ">
      {/* Summary cards */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 md:cols-gap-3 mr-2 w-full">
        {[
          {
      ...
      <div className="overflow-x-scroll md:overflow-x-hidden">
        <div className="rounded-2xl border border-border bg-card w-fit md:w-full">
          {/* Filters */}
          ...
          <div className="divide-y divide-border">
            {visibleBookings.map((b) => {
              ...
              return (
                <div
                  key={b.id}
                  className="group px-5 py-4 hover:bg-white/3 transition-all"
                >
                  {/* Desktop */}
                  <div
                    className="hidden sm:grid items-center"
                    style={{
                      gridTemplateColumns: "1fr 160px 90px 110px 100px",
                    }}
                  >
                  ...
                  <div className="sm:hidden flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <span
                          className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold flex-shrink-0 ${st.bg} ${st.text}`}
                        >
      ...
      <div className="p-5 rounded-2xl border border-border bg-card">
        ...
        <div className="grid grid-cols-7 gap-1">
          {[null, null, 1, ...Array.from({ length: 29 }, (_, i) => i + 2)].map(
            (d, i) => {
              ...
              return (
                <div
                  ...
                  style={
                    booked
                      ? {
                          background:
                            "linear-gradient(135deg, rgba(255,107,0,0.25), rgba(255,34,0,0.15))",
                ...
                >
                  ...
                  {(booked || pending) && (
                    <span
                      className={`absolute bottom-0.5 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full ${booked ? "bg-primary" : "bg-chart-3"}`}
                    />
                  )}
                </div>
              );
