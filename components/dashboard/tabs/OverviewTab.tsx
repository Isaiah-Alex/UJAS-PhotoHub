...
import { CalendarDays, CheckCircle, ChevronRight, Eye, ShoppingCart, Wallet } from "lucide-react";
import { UImg } from "@/components/photohub/Helpers";
import { RevenueChart } from "@/components/dashboard/charts/RevenueChart";
import { SparkBar } from "@/components/dashboard/charts/SparkBar";
import { statusStyle, activityColor, activityEmoji } from "@/components/dashboard/dashboard-constants";
...
export function OverviewTab({
    ...
    const upcoming = dashBookings.filter((b) => b.status !== "completed").slice(0, 4);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                    {
                        label: "Total Earnings", value: "₦24.6M", sub: "+18.4% this month",
                        icon: <Wallet size={18} />, color: "var(--primary)",
                        spark: [280, 320, 290, 410, 380, 520, 470, 610],
                    },
                    {
                        label: "Bookings", value: `${dashBookings.length}`, sub: `${confirmedCount} confirmed`,
                        icon: <CalendarDays size={18} />, color: "var(--chart-2)",
                        spark: [5, 8, 6, 11, 9, 14, 12, 17],
                    },
                    {
                        label: "Profile Views", value: "12,840", sub: "+24% this week",
                        icon: <Eye size={18} />, color: "var(--chart-4)",
                        spark: [80, 110, 95, 140, 125, 160, 138, 184],
                    },
                    {
                        label: "Photo Sales", value: "284", sub: "+₦1.2M revenue",
                        icon: <ShoppingCart size={18} />, color: "var(--chart-3)",
                    ...
                    <div key={label} className="p-5 rounded-2xl border border-border bg-card space-y-3">
                        <div className="flex items-center justify-between">
                            <div
                                ...
                                style={{ background: `color-mix(in srgb, ${color} 15%, transparent)`, border: `1px solid color-mix(in srgb, ${color} 22%, transparent)` }}
                            >
            ...
            <div className="grid lg:grid-cols-[1fr_300px] gap-4">
                                ...
                                return (
                                    <button
                                        ...
                                        className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                                            revenueView === key ? "bg-white/10 text-white" : "text-white/35 hover:text-white"
                                        }`}
                                    >
                                        {v}
                                    </button>
                ...
                <div className="p-5 rounded-2xl border border-border bg-card">
                        ...
                        {activityFeed.map((a, i) => (
                            <div key={i} className="flex items-start gap-3">
                                <div
                                    ...
                                    style={{ background: `color-mix(in srgb, ${activityColor[a.type]} 18%, transparent)` }}
                                >
                    ...
                    <span className="text-[10px] font-semibold" style={{ color: activityColor[a.type] }}>
                      {a.sub}
                    </span>
            ...
            <div className="grid lg:grid-cols-[1fr_280px] gap-4">
                ...
                <div className="p-5 rounded-2xl border border-border bg-card">
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="font-display font-bold text-white text-sm">Upcoming Sessions</h3>
                        <button
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-[11px] text-muted-foreground">{b.type} · {b.date}</p>
                                    </div>
                ...
                <div className="p-5 rounded-2xl border border-border bg-card space-y-4">
                    <h3 className="font-display font-bold text-white text-sm">Profile Strength</h3>
                    <div className="space-y-3">
                        {[
                            ...
                            <div key={item.label} className="flex items-center gap-2.5">
                                <div
                                    ...
                                    style={item.done ? { background: "linear-gradient(135deg, var(--chart-2), #2ab866)" } : {}}
                                >
                                    {item.done && <CheckCircle size={10} className="text-white" />}
                                </div>
                                <span className={`text-xs ${item.done ? "text-white/65" : "text-white/30"}`}>
                  {item.label}
                </span>
                            </div>
                        ))}
