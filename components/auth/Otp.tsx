...
export function OtpStep({
  ...
  onVerified: (role: "client" | "photographer") => void;
  onBack: () => void;
}) {
    ...
    <form onSubmit={handleVerify} className="space-y-5">
      ...
      <div>
        ...
        <p className="text-white/40 text-sm leading-relaxed">
          ...
          <span className="text-white/70 font-medium">{email}</span>. Enter it
          below to continue.
        </p>
      </div>

      <div
        className="flex gap-2.5 pt-1 w-auto overflow-hidden"
        onPaste={handlePaste}
      >
        {otp.map((digit, i) => (
          <input
