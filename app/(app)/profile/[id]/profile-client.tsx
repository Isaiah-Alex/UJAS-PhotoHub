...
import { UImg, Stars, CoverImg } from "@/components/photohub/Helpers";
import {
  Award,
  MapPin,
  Users,
...
export function ProfileClient({ id }: Props) {
  ...
  return (
    <div className="min-h-screen ">
      {/* Cover */}
      <div className="relative h-72 md:h-[380px] w-screen">
        <CoverImg name={p.cover} alt="Cover" className="w-screen h-full" />
        <div
          className="absolute inset-0"
          style={{
            background:
              "linear-gradient(to bottom, rgba(5,5,5,0.1) 0%, rgba(5,5,5,0.6) 70%, var(--background) 100%)",
          }}
        />
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 -mt-24 relative z-10">
          ...
          <div className="flex-1">
            <div className="flex flex-wrap items-start justify-between gap-4">
              ...
              <div className="flex gap-2.5 ">
                <GlassBtn className="px-5 py-2.5 rounded-xl text-sm">
                  Follow
                </GlassBtn>
                <GradBtn
                  onClick={() => router.push(`${pathname}/booking`)}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8 w-full">
          {[
            ...
            {
              label: "Rate",
              value: formatHourlyRate(p.priceHourly),
              Icon: Banknote,
              color: "var(--chart-4)",
            },
            ...
            <div
              key={label}
              className="p-4 rounded-2xl bg-muted border border-white/5"
            >
          ...
          <div className="max-w-2xl space-y-5 mb-14">
            <div className="p-6 rounded-2xl bg-muted border border-white/5">
              ...
              <p className="text-white/55 leading-relaxed text-sm">
                {p.bio} With a keen eye for detail and an intuitive
                understanding of light, {p.name.split(" ")[0]} has built a
                reputation for creating photographs that tell deeply personal
                stories. Work featured in exhibitions across Lagos, Abuja,
            ...
            <div className="p-6 rounded-2xl bg-muted border border-white/5">
              ...
              <div className="flex flex-wrap gap-2">
                {p.specializations.map((s) => (
                  <span
                    key={s}
                    className="px-3 py-1.5 rounded-lg bg-white/5 text-sm text-white/55 border border-white/8"
                  >
                    {s}
                  </span>
                ))}
        ...
        {activeTab === "Reviews" && (
          <div className="space-y-4 mb-14 max-w-2xl">
            {reviews.map((t) => (
              <div
                key={t.id}
                className="p-5 rounded-2xl bg-muted border border-white/5"
              >
            ...
            {marketplaceItems.map((item) => (
              ...
              >
                ...
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center">
                  <GradBtn className="px-5 py-2.5 rounded-xl text-sm">
                    Buy {formatMoney(item.priceAmount)}
                  </GradBtn>
                </div>
                <div
